...
class Plotter(object):  
    def __init__(self, epochs, prior_pop):   
        self.t = 0
        ...
        gs = gridspec.GridSpec(s + h, s*3 + w)
        
        self.losses_ax = self.fig.add_subplot(gs[1:h-4, 2:s*3+w-2])
        ...
        self.prior_points, = self.hidden_ax.plot(
                prior_pop[:,0],prior_pop[:,1] , "o",
                markersize=1, lw=0, color="black",label="prior", alpha=0.1)
        self.hidden_plots = []
        for x in range(10):
        ...
        for x in range(s):
            for y in range(s):
                ax = self.fig.add_subplot(gs[h + s - x - 2, s + w*1/3 + y])
        ...
        self.decoded_pattern_axes = []
        self.decoded_pattern_imgs = []
        for x in range(s):
            for y in range(s):
                ax = self.fig.add_subplot(gs[h + s - x - 2, s*2 + w*2/3 + y])
                im = ax.imshow(np.zeros([img_side, img_side]), 
                               vmin=-1, vmax=1)
                ax.get_xaxis().set_visible(False)
                ax.get_yaxis().set_visible(False)
                self.decoded_pattern_axes.append(ax)
                self.decoded_pattern_imgs.append(im)
    ...
    def plot(self, R_loss, hidden, labels, grid_patterns, generated_patterns):
        ...
        for x in range(self.s):
            for y in range(self.s):
                ...
                im.set_data(grid_patterns[k].reshape(img_side, img_side))
                im = self.decoded_pattern_imgs[k]
                im.set_data(generated_patterns[k].reshape(img_side, img_side))

        self.fig.canvas.draw()
        self.fig.savefig("imgs/acae.png".format(self.t))
        self.fig.savefig("imgs/acae-{:03d}.png".format(self.t))
        self.t += 1
